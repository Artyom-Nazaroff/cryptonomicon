			...
			<template v-if="tickers.length">
				...
				<div>
					<button
						@click="page = page - 1"
						:disabled="page <= 1"
						class="my-4 inline-flex items-center py-2 px-4 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-full text-white bg-gray-600 hover:bg-gray-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 mr-4"
					>
						Назад
					</button>
					<button
...
export default {
	...
	async mounted() {
		try {
			...
			Object.entries(allCoins.Data).forEach(i => {
				const [key, val] = i
				this.allCoinsList.push([key, val.FullName])
			})
		} catch (e) {
	...
	watch: {
		...
		pageStateOptions(val) {
			history.pushState(null, document.title, `${window.location.pathname}?filter=${val.filter}&page=${val.page}`)
		},
		ticker(val) {
			this.coincidences = []
			...
			if (val.length) {
				this.allCoinsList.forEach(i => {
					if (i[1].toLowerCase().includes(val.toLowerCase()) && this.coincidences.length <= 3) {
	...
	computed: {
		...
		normalizedGraph() {
			...
			if (maxVal === minVal) return this.graph.map(() => 50)
	...
	methods: {
		...
		add() {
			...
			if (!this.isError) {
				// Здесь пойдем по пути Реакта, чтобы обновить ссылку. Это нужно для срабатывания вотчера
